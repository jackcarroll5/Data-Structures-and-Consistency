...
public class AVLTree<E extends Comparable<E>> {
    ...
    public boolean contains(E word) { //Recursive
        Node current = root;
        return containsSub(root, word);
    }

    private boolean containsSub(Node node, E word) { //Recursive
        if (node == null)
            return false;
        else {
    ...
    /*MUST BE FILLED*/
    public boolean containsIterative(E word) //Iterative
    {
       Node current = root;

       while (current != null)
       {
         int compare = word.compareTo(current.key);
